...
void CacheLevels()
{
	for (;;)
	{
		...
		for (DWORD i = 0; i < levelCount; ++i)
		{
			...
			DWORD actorCount = read<DWORD>(ItemLevel + (FortniteOffsets::Globals::AActors + sizeof(PVOID)));
			uintptr_t ItemsPawns = read<uintptr_t>(ItemLevel + FortniteOffsets::Globals::AActors);
