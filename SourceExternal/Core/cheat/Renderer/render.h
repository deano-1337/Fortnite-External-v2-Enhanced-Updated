...
namespace menu
{
    inline void Render()
    {
        ...
        if (ImGui::BeginTabBar("##tabs"))
        {
            ...
            {
                ImGui::Text("Exploits Options");

                ImGui::Checkbox("Teleport Enemy", &settings::exploits.tpenemy);
                ImGui::Checkbox("Flying", &settings::exploits.flyy);
                ImGui::SliderFloat("Fly Speed", &settings::exploits.flySpeed, 1.0f, 50.0f);

                ImGui::Checkbox("Free Cam", &settings::exploits.FreeCam);
                ImGui::Checkbox("Speed Hack", &settings::exploits.speedhack);
                //ImGui::Checkbox("Bullet Time / BTrack", &settings::exploits.btrack);
                ImGui::Checkbox("First Person", &settings::exploits.firstperson);
                ImGui::Checkbox("Double Pump", &settings::exploits.doublepump);
                ImGui::Checkbox("Instant Reload", &settings::exploits.instaReloadd);

                // Optional / commented ones you had before
                // ImGui::Checkbox("Invisible", &settings::exploits.invisible);
                // ImGui::Checkbox("Magic", &settings::exploits.magic);

                // Silent Aim section
                ImGui::Checkbox("Silent Aimbot", &settings::exploits.silentaim);
                if (settings::exploits.silentaim)
                {
                    settings::aimbot.mouse = false;
                    settings::aimbot.enabled = false;
                    settings::exploits.silentaim = true;

                    ImGui::Checkbox("Enable FOV", &settings::aimbot.drawfov);
                    if (settings::aimbot.drawfov)
                    {
                        ImGui::SliderInt("FOV Size", &settings::exploits.fovsize, 1, 1000);
                    }

                    ImGui::Checkbox("Enable Visible Check", &settings::aimbot.vischeck);
                    ImGui::Checkbox("Ignore Team (Visual too)", &settings::aimbot.ignore_team);

                    const char* hitboxes[5] = { "Head", "Neck", "Chest", "Pelvis", "Random" };
                    ImGui::Combo("Target Hitbox", &settings::aimbot.Hitbox, hitboxes, IM_ARRAYSIZE(hitboxes), 2);

                    const char* silentTargetPriorities[2] = { "Closest to Me", "Closest to Crosshair" };
                    ImGui::Combo("Silent Aim Target Priority", &settings::exploits.silentTargetPriority, silentTargetPriorities, IM_ARRAYSIZE(silentTargetPriorities), 2);
                }
